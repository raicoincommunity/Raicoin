...
rai::TokenMapKey::TokenMapKey(const rai::Account& to, rai::Chain chain,
                              uint64_t height, uint64_t index)
    : to_(to), chain_(chain), height_(height), index_(index)
{
}

void rai::TokenMapKey::Serialize(rai::Stream& stream) const
{
    rai::Write(stream, to_.bytes);
    rai::Write(stream, chain_);
    rai::Write(stream, ~height_);
...
bool rai::TokenMapKey::Deserialize(rai::Stream& stream)
{
    bool error = false;
    error = rai::Read(stream, to_.bytes);
    IF_ERROR_RETURN(error, true);
...
rai::TokenMapInfo::TokenMapInfo()
    : source_tx_(0), from_(0), value_(0), target_tx_(0)
{
}

rai::TokenMapInfo::TokenMapInfo(const rai::TokenKey& token,
                                const rai::BlockHash& source_tx_hash,
                                const rai::Account& from,
                                const rai::TokenValue& value)
    : token_(token),
      source_tx_(source_tx_hash),
      from_(from),
      value_(value),
      target_tx_(0)
{
...
void rai::TokenMapInfo::Serialize(rai::Stream& stream) const
{
    token_.Serialize(stream);
...
bool rai::TokenMapInfo::Deserialize(rai::Stream& stream)
{
    bool error = false;
    error = token_.Deserialize(stream);
    IF_ERROR_RETURN(error, true);
    error = rai::Read(stream, source_tx_.bytes);
    IF_ERROR_RETURN(error, true);
    error = rai::Read(stream, from_.bytes);
    IF_ERROR_RETURN(error, true);
    error = rai::Read(stream, value_.bytes);
    IF_ERROR_RETURN(error, true);
    error = rai::Read(stream, target_tx_.bytes);
    IF_ERROR_RETURN(error, true);
    return false;
}

rai::TokenUnmapInfo::TokenUnmapInfo()
    : type_(rai::TokenType::INVALID),
      value_(0),
      to_(0),
      source_tx_(0),
      extra_data_(0),
      target_tx_(0),
      target_height_(rai::Block::INVALID_HEIGHT)
{
}

rai::TokenUnmapInfo::TokenUnmapInfo(const rai::TokenKey& token,
                                    rai::TokenType type,
                                    const rai::TokenValue& value,
                                    const rai::Account& to,
                                    const rai::BlockHash& source_tx,
                                    uint64_t extra_data)
    : token_(token),
      type_(type),
      value_(value),
      to_(to),
      source_tx_(source_tx),
...
void rai::TokenUnmapInfo::Serialize(rai::Stream& stream) const
{
    token_.Serialize(stream);
    rai::Write(stream, type_);
    rai::Write(stream, value_.bytes);
...
bool rai::TokenUnmapInfo::Deserialize(rai::Stream& stream)
{
    bool error = false;
    error = token_.Deserialize(stream);
    IF_ERROR_RETURN(error, true);
...
bool rai::Ledger::TokenMapInfoPut(rai::Transaction& transaction,
                                  const rai::TokenMapKey& key,
                                  const rai::TokenMapInfo& info)
{
    ...
    rai::MdbVal mdb_key(bytes_key.size(), bytes_key.data());
    rai::MdbVal mdb_value(bytes_value.size(), bytes_value.data());
    bool error = store_.Put(transaction.mdb_transaction_, store_.token_map_,
                            mdb_key, mdb_value);
...
bool rai::Ledger::TokenMapInfoGet(rai::Transaction& transaction,
                                  const rai::TokenMapKey& key,
                                  rai::TokenMapInfo& info) const
{
    std::vector<uint8_t> bytes_key;
    {
        rai::VectorStream stream(bytes_key);
    ...
    rai::MdbVal mdb_key(bytes_key.size(), bytes_key.data());
    rai::MdbVal mdb_value;
    bool error = store_.Get(transaction.mdb_transaction_, store_.token_map_,
                            mdb_key, mdb_value);
    IF_ERROR_RETURN(error, error);
    rai::BufferStream stream(mdb_value.Data(), mdb_value.Size());
    return info.Deserialize(stream);
}

bool rai::Ledger::TokenMapInfoGet(const rai::Iterator& it,
                                  rai::TokenMapKey& key,
                                  rai::TokenMapInfo& info) const
{
    ...
    bool error = key.Deserialize(stream_key);
    IF_ERROR_RETURN(error, true);
...
bool rai::Ledger::TokenMapInfoExists(rai::Transaction& transaction,
                                     const rai::TokenMapKey& key) const
{
    rai::TokenMapInfo info;
    bool error = TokenMapInfoGet(transaction, key, info);
    return !error;
}

rai::Iterator rai::Ledger::TokenMapInfoLowerBound(rai::Transaction& transaction,
                                                  const rai::Account& to,
                                                  rai::Chain chain,
                                                  uint64_t height,
                                                  uint64_t index) const
{
    rai::TokenMapKey key(to, chain, height, index);
    std::vector<uint8_t> bytes_key;
    {
        rai::VectorStream stream(bytes_key);
        key.Serialize(stream);
    ...
    rai::MdbVal mdb_key(bytes_key.size(), bytes_key.data());
    rai::StoreIterator store_it(transaction.mdb_transaction_, store_.token_map_,
                                mdb_key);
    return rai::Iterator(std::move(store_it));
}

rai::Iterator rai::Ledger::TokenMapInfoUpperBound(rai::Transaction& transaction,
                                                  const rai::Account& to,
...
bool rai::Ledger::TokenMapIndexPut(rai::Transaction& transaction,
                                   rai::Chain chain, const rai::BlockHash& hash,
                                   uint64_t index)
{
    if (!transaction.write_)
    {
    ...
    {
        rai::VectorStream stream(bytes_value);
        rai::Write(stream, index);
    }

    rai::MdbVal mdb_key(bytes_key.size(), bytes_key.data());
    rai::MdbVal mdb_value(bytes_value.size(), bytes_value.data());
    bool error = store_.Put(transaction.mdb_transaction_,
                            store_.token_map_index_, mdb_key, mdb_value);
    IF_ERROR_RETURN(error, error);

    return false;
}

bool rai::Ledger::TokenMapIndexGet(rai::Transaction& transaction,
                                   rai::Chain chain, const rai::BlockHash& hash,
                                   uint64_t& index)
{
    std::vector<uint8_t> bytes_key;
                                    ...
                                    const rai::TokenUnmapInfo& info)
{
    ...
    bool error = store_.Put(transaction.mdb_transaction_, store_.token_unmap_,
                            key, value);
    IF_ERROR_RETURN(error, true);
...
bool rai::Ledger::TokenUnmapInfoGet(rai::Transaction& transaction,
                                    const rai::Account& account,
                                    uint64_t height,
                                    rai::TokenUnmapInfo& info) const
{
    std::vector<uint8_t> bytes_key;
    {
        rai::VectorStream stream(bytes_key);
    ...
    rai::MdbVal key(bytes_key.size(), bytes_key.data());
    rai::MdbVal value;
    bool error = store_.Get(transaction.mdb_transaction_, store_.token_unmap_,
                            key, value);
    IF_ERROR_RETURN(error, true);
...
bool rai::Ledger::TokenUnmapInfoGet(const rai::Iterator& it,
...
rai::Iterator rai::Ledger::TokenUnmapInfoLowerBound(
    ...
    uint64_t height) const
{
    ...
    rai::StoreIterator store_it(transaction.mdb_transaction_,
                                store_.token_unmap_, key);
    return rai::Iterator(std::move(store_it));
}

rai::Iterator rai::Ledger::TokenUnmapInfoUpperBound(
...
rai::TokenWrapInfo::TokenWrapInfo(const rai::TokenKey& token,
                                  rai::TokenType type,
                                  const rai::TokenValue& value,
                                  const rai::Chain to_chain,
                                  const rai::Account& to_account,
                                  const rai::BlockHash& source_tx)
    : token_(token),
      type_(type),
      value_(value),
...
void rai::TokenWrapInfo::Serialize(rai::Stream& stream) const
{
    token_.Serialize(stream);
    rai::Write(stream, type_);
    rai::Write(stream, value_.bytes);
...
bool rai::TokenWrapInfo::Deserialize(rai::Stream& stream)
{
    ...
    error = rai::Read(stream, value_.bytes);
    IF_ERROR_RETURN(error, true);
    error = rai::Read(stream, to_chain_);
    IF_ERROR_RETURN(error, true);
...
bool rai::Ledger::TokenWrapInfoPut(rai::Transaction& transaction,
                                   const rai::Account& account, uint64_t height,
                                   const rai::TokenWrapInfo& info)
{
    if (!transaction.write_)
    {
        return true;
    ...
    rai::MdbVal key(bytes_key.size(), bytes_key.data());
    rai::MdbVal value(bytes_value.size(), bytes_value.data());
    bool error = store_.Put(transaction.mdb_transaction_, store_.token_wrap_,
                            key, value);
    IF_ERROR_RETURN(error, true);
...
bool rai::Ledger::TokenWrapInfoGet(rai::Transaction& transaction,
                                   const rai::Account& account, uint64_t height,
                                   rai::TokenWrapInfo& info) const
{
    std::vector<uint8_t> bytes_key;
    {
        rai::VectorStream stream(bytes_key);
    ...
    rai::MdbVal key(bytes_key.size(), bytes_key.data());
    rai::MdbVal value;
    bool error = store_.Get(transaction.mdb_transaction_, store_.token_wrap_,
                            key, value);
    IF_ERROR_RETURN(error, true);
...
bool rai::Ledger::TokenWrapInfoGet(const rai::Iterator& it,
                                    rai::Account& account, uint64_t& height,
                                    rai::TokenWrapInfo& info) const
{
    auto data = it.store_it_->first.Data();
...
rai::Iterator rai::Ledger::TokenWrapInfoLowerBound(
    rai::Transaction& transaction, const rai::Account& account,
    uint64_t height) const
{
    std::vector<uint8_t> bytes_key;
    ...
    rai::MdbVal key(bytes_key.size(), bytes_key.data());
    rai::StoreIterator store_it(transaction.mdb_transaction_,
                                store_.token_wrap_, key);
    return rai::Iterator(std::move(store_it));
}

rai::Iterator rai::Ledger::TokenWrapInfoUpperBound(
    rai::Transaction& transaction, const rai::Account& account) const
{
    rai::Account account_next(account + 1);
    if (account_next.IsZero())
    ...
    return TokenWrapInfoLowerBound(transaction, account_next);
}
...
rai::MakerSwapIndex::MakerSwapIndex()
    : maker_(0),
      trade_height_(rai::Block::INVALID_HEIGHT),
      main_account_(0),
...
rai::MakerSwapIndex::MakerSwapIndex(const rai::Account& maker,
                                    uint64_t trade_height,
                                    const rai::Account& main_account,
                                    uint64_t inquiry_ack_height)
    : maker_(maker),
      trade_height_(trade_height),
      main_account_(main_account),
      inquiry_ack_height_(inquiry_ack_height)
{
}

void rai::MakerSwapIndex::Serialize(rai::Stream& stream) const
{
    rai::Write(stream, maker_.bytes);
    rai::Write(stream, ~trade_height_);
    rai::Write(stream, main_account_.bytes);
    rai::Write(stream, inquiry_ack_height_);
}

bool rai::MakerSwapIndex::Deserialize(rai::Stream& stream)
{
    bool error = false;
    error = rai::Read(stream, maker_.bytes);
    IF_ERROR_RETURN(error, true);
    ...
    IF_ERROR_RETURN(error, true);
    trade_height_ = ~trade_height_;
    error = rai::Read(stream, main_account_.bytes);
    IF_ERROR_RETURN(error, true);
...
rai::WrappedTokenInfo::WrappedTokenInfo()
    : original_chain_(rai::Chain::INVALID),
      original_address_(0),
      block_height_(rai::Block::INVALID_HEIGHT),
      tx_hash_(0)
{
...
rai::WrappedTokenInfo::WrappedTokenInfo(
    ...
    uint64_t block_height, const rai::BlockHash& tx_hash)
...
void rai::WrappedTokenInfo::Serialize(rai::Stream& stream) const
{
    rai::Write(stream, original_chain_);
    rai::Write(stream, original_address_.bytes);
    rai::Write(stream, block_height_);
    rai::Write(stream, tx_hash_.bytes);
}

bool rai::WrappedTokenInfo::Deserialize(rai::Stream& stream)
{
    bool error = false;
    error = rai::Read(stream, original_chain_);
    IF_ERROR_RETURN(error, true);
    error = rai::Read(stream, original_address_.bytes);
    IF_ERROR_RETURN(error, true);
    error = rai::Read(stream, block_height_);
    IF_ERROR_RETURN(error, true);
    error = rai::Read(stream, tx_hash_.bytes);
...
bool rai::Ledger::TokenUnwrapInfoPut(rai::Transaction& transaction,
                                     const rai::TokenUnwrapKey& key,
                                     const rai::TokenUnwrapInfo& info)
{
    if (!transaction.write_)
    {
        return true;
    ...
    rai::MdbVal mdb_key(bytes_key.size(), bytes_key.data());
    rai::MdbVal mdb_value(bytes_value.size(), bytes_value.data());
    bool error = store_.Put(transaction.mdb_transaction_, store_.token_unwrap_,
                            mdb_key, mdb_value);
    IF_ERROR_RETURN(error, error);
...
bool rai::Ledger::TokenUnwrapInfoGet(rai::Transaction& transaction,
                                     const rai::TokenUnwrapKey& key,
                                     rai::TokenUnwrapInfo& info) const
{
    std::vector<uint8_t> bytes_key;
    {
        rai::VectorStream stream(bytes_key);
    ...
    rai::MdbVal mdb_key(bytes_key.size(), bytes_key.data());
    rai::MdbVal mdb_value;
    bool error = store_.Get(transaction.mdb_transaction_, store_.token_unwrap_,
                            mdb_key, mdb_value);
    IF_ERROR_RETURN(error, error);
    rai::BufferStream stream(mdb_value.Data(), mdb_value.Size());
    return info.Deserialize(stream);
...
bool rai::Ledger::TokenUnwrapInfoGet(const rai::Iterator& it,
                                     rai::TokenUnwrapKey& key,
                                     rai::TokenUnwrapInfo& info) const
{
    auto data = it.store_it_->first.Data();
    auto size = it.store_it_->first.Size();
...
bool rai::Ledger::TokenUnwrapInfoExists(rai::Transaction& transaction,
                                        const rai::TokenUnwrapKey& key) const
{
    rai::TokenUnwrapInfo info;
    bool error = TokenUnwrapInfoGet(transaction, key, info);
    return !error;
}

rai::Iterator rai::Ledger::TokenUnwrapInfoLowerBound(
    rai::Transaction& transaction, const rai::Account& to, rai::Chain chain,
    uint64_t height, uint64_t index) const
{
    rai::TokenUnwrapKey key(to, chain, height, index);
    std::vector<uint8_t> bytes_key;
    {
        rai::VectorStream stream(bytes_key);
        key.Serialize(stream);
    ...
    rai::MdbVal mdb_key(bytes_key.size(), bytes_key.data());
    rai::StoreIterator store_it(transaction.mdb_transaction_,
                                store_.token_unwrap_, mdb_key);
    return rai::Iterator(std::move(store_it));
}

rai::Iterator rai::Ledger::TokenUnwrapInfoUpperBound(
    rai::Transaction& transaction, const rai::Account& to,
    rai::Chain chain) const
{
    uint32_t next = static_cast<uint32_t>(chain) + 1;
    rai::Account account(to);
    ...
    return TokenUnwrapInfoLowerBound(transaction, account,
                                     static_cast<rai::Chain>(next));
}

bool rai::Ledger::TokenUnwrapIndexPut(rai::Transaction& transaction,
                                      rai::Chain chain,
                                      const rai::BlockHash& hash,
                                      uint64_t index)
{
    ...
    rai::MdbVal mdb_value(bytes_value.size(), bytes_value.data());
    bool error = store_.Put(transaction.mdb_transaction_,
                            store_.token_unwrap_index_, mdb_key, mdb_value);
    IF_ERROR_RETURN(error, error);

    return false;
}

bool rai::Ledger::TokenUnwrapIndexGet(rai::Transaction& transaction,
                                      rai::Chain chain,
                                      const rai::BlockHash& hash,
                                      uint64_t& index)
{
...
bool rai::Ledger::MakerSwapIndexPut(rai::Transaction& transaction,
                                    const rai::MakerSwapIndex& index,
                                    const rai::Account& taker,
                                    uint64_t inquiry_height)
{
    if (!transaction.write_)
    ...
    {
        rai::VectorStream stream(bytes_value);
        rai::Write(stream, taker.bytes);
        rai::Write(stream, inquiry_height);
    }

    rai::MdbVal key(bytes_key.size(), bytes_key.data());
    rai::MdbVal value(bytes_value.size(), bytes_value.data());
    bool error = store_.Put(transaction.mdb_transaction_,
                            store_.maker_swap_index_, key, value);
    IF_ERROR_RETURN(error, error);

    return false;
}

bool rai::Ledger::MakerSwapIndexGet(const rai::Iterator& it,
                                    rai::MakerSwapIndex& index,
                                    rai::Account& taker,
                                    uint64_t& inquiry_height) const
{
    auto data = it.store_it_->first.Data();
    auto size = it.store_it_->first.Size();
    if (data == nullptr || size == 0)
    ...
    rai::BufferStream stream_value(data, size);
    error = rai::Read(stream_value, taker.bytes);
    IF_ERROR_RETURN(error, true);
    error = rai::Read(stream_value, inquiry_height);
    IF_ERROR_RETURN(error, true);

    return false;
}

rai::Iterator rai::Ledger::MakerSwapIndexLowerBound(
    rai::Transaction& transaction, const rai::Account& maker,
    uint64_t trade_height) const
{
    rai::MakerSwapIndex index(maker, trade_height, rai::Account(0), 0);
    std::vector<uint8_t> bytes_key;
    {
        rai::VectorStream stream(bytes_key);
        index.Serialize(stream);
    ...
    rai::MdbVal key(bytes_key.size(), bytes_key.data());
    rai::StoreIterator store_it(transaction.mdb_transaction_,
                                store_.maker_swap_index_, key);
    return rai::Iterator(std::move(store_it));
}

rai::Iterator rai::Ledger::MakerSwapIndexUpperBound(
    rai::Transaction& transaction, const rai::Account& maker) const
{
    rai::Account next = maker + 1;
    if (next.IsZero())
    {
        return rai::Iterator();
    }

    return MakerSwapIndexLowerBound(transaction, next,
                                    std::numeric_limits<uint64_t>::max());
}

bool rai::Ledger::WrappedTokenInfoPut(rai::Transaction& transaction,
                                      const rai::TokenKey& key,
                                      const rai::WrappedTokenInfo& info)
{
    if (!transaction.write_)
    {
        return true;
    ...
    rai::MdbVal mdb_value(bytes_value.size(), bytes_value.data());
    bool error = store_.Put(transaction.mdb_transaction_,
                            store_.wrapped_tokens_, mdb_key, mdb_value);
    IF_ERROR_RETURN(error, error);

    return false;
}

bool rai::Ledger::WrappedTokenInfoGet(rai::Transaction& transaction,
                                      const rai::TokenKey& key,
                                      rai::WrappedTokenInfo& info) const
{
    std::vector<uint8_t> bytes_key;
    {
        rai::VectorStream stream(bytes_key);
